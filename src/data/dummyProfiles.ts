export interface AspirantProfile {
  id: string;
  name: string;
  age: number;
  city: string;
  rank: string;
  status: 'Imperial' | 'Vanguard' | 'Sealed' | 'Rising';
  bio: string;
  img: string;
  height?: number; // Used for masonry layout calculation
  height_str: string; // e.g. 6'2"
  vocation: string; // e.g. AI Logistics Founder
  tier: string; // e.g. Sovereign Asset Tier
  is_verified?: boolean;
}

export const DUMMY_ASPIRANTS: AspirantProfile[] = [
  {
    id: 'm1',
    name: 'Vikram Singh',
    age: 28,
    city: 'New Delhi',
    rank: 'Percentile 0.1%',
    status: 'Imperial',
    bio: 'Founder of an AI logistics startup. Alumnus of IIT Delhi. Loves trekking in Ladakh.',
    img: 'https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?q=80&w=1287&auto=format&fit=crop',
    height: 1600,
    height_str: "6'2\"",
    vocation: 'AI Logistics Founder',
    tier: 'Imperial Asset Tier'
  },
  {
    id: 'm2',
    name: 'Arjun Mehra',
    age: 31,
    city: 'Mumbai',
    rank: 'Percentile 1.2%',
    status: 'Vanguard',
    bio: 'Investment banker with a passion for classical piano. Architect of his own life.',
    img: 'https://images.unsplash.com/photo-1519085360753-af0119f7cbe7?q=80&w=1287&auto=format&fit=crop',
    height: 1400,
    height_str: "5'11\"",
    vocation: 'Investment Banker',
    tier: 'Vanguard Equity Tier'
  },
  {
    id: 'm3',
    name: 'Kabir Varma',
    age: 29,
    city: 'Bangalore',
    rank: 'Percentile 4.5%',
    status: 'Sealed',
    bio: 'Architectural designer. Believes in minimal living and maximal thinking.',
    img: 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?q=80&w=1287&auto=format&fit=crop',
    height: 1800,
    height_str: "6'4\"",
    vocation: 'Architectural Designer',
    tier: 'Prime Capital Tier'
  },
  {
    id: 'm4',
    name: 'Rohan Malhotra',
    age: 27,
    city: 'Pune',
    rank: 'Percentile 10.2%',
    status: 'Rising',
    bio: 'Data scientist and competitive chess player. Seeking a resonance of the mind.',
    img: 'https://images.unsplash.com/photo-1552374196-c4e7ffc6e126?q=80&w=1287&auto=format&fit=crop',
    height: 1500,
    height_str: "5'10\"",
    vocation: 'Data Scientist',
    tier: 'Emergent Signal Tier'
  },
  {
    id: 'm5',
    name: 'Dev Advani',
    age: 30,
    city: 'Dubai',
    rank: 'Percentile 2.3%',
    status: 'Imperial',
    bio: 'International corporate lawyer. World traveler. Seeking a sovereign partner.',
    img: 'https://images.unsplash.com/photo-1531384441138-2736e62e0919?q=80&w=1287&auto=format&fit=crop',
    height: 1700,
    height_str: "6'1\"",
    vocation: 'Corporate Lawyer',
    tier: 'Global Asset Tier',
    is_verified: true
  },
  {
    id: 'm6',
    name: 'Neel Kanth',
    age: 26,
    city: 'Hyderabad',
    rank: 'Percentile 45.0%',
    status: 'Rising',
    bio: 'Software engineer at a FAANG company. Weekend baker and cyclist.',
    img: 'https://images.unsplash.com/photo-1528892952291-009c663ce843?q=80&w=1287&auto=format&fit=crop',
    height: 1450,
    height_str: "5'9\"",
    vocation: 'Software Engineer',
    tier: 'Ascent Signal Tier'
  },
  {
    id: 'm7',
    name: 'Aryan Goel',
    age: 32,
    city: 'Delhi',
    rank: 'Percentile 8.9%',
    status: 'Vanguard',
    bio: 'Real estate developer. Collector of vintage watches. Guided by tradition.',
    img: 'https://images.unsplash.com/photo-1618077360395-f3068be8e001?q=80&w=1287&auto=format&fit=crop',
    height: 1650,
    height_str: "6'0\"",
    vocation: 'Real Estate Developer',
    tier: 'Legacy Asset Tier'
  },
  {
    id: 'm8',
    name: 'Dhruv Kapoor',
    age: 29,
    city: 'Chandigarh',
    rank: 'Entry (1,204)',
    status: 'Rising',
    bio: 'Public policy researcher. Believes in the power of conversation.',
    img: 'https://images.unsplash.com/photo-1492562080023-ab3db95bfbbce?q=80&w=1287&auto=format&fit=crop',
    height: 1550,
    height_str: "5'11\"",
    vocation: 'Policy Researcher',
    tier: 'Civic Signal Tier'
  },
  {
    id: 'm9',
    name: 'Ishaan Reddy',
    age: 28,
    city: 'Bangalore',
    rank: 'Percentile 1.5%',
    status: 'Imperial',
    bio: 'Automotive engineer. Raced in Formula 3. Precision is my lifestyle.',
    img: 'https://images.unsplash.com/photo-1560250097-0b93528c311a?q=80&w=1287&auto=format&fit=crop',
    height: 1600,
    height_str: "6'2\"",
    vocation: 'Automotive Engineer',
    tier: 'Supreme Precision Tier'
  },
  {
    id: 'm10',
    name: 'Siddharth Jain',
    age: 33,
    city: 'Mumbai',
    rank: 'Percentile 3.2%',
    status: 'Vanguard',
    bio: 'Art gallery owner. Searching for the ultimate aesthetic resonance.',
    img: 'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?q=80&w=1287&auto=format&fit=crop',
    height: 1750,
    height_str: "6'1\"",
    vocation: 'Art Gallery Owner',
    tier: 'Aesthetic Equity Tier'
  },
  {
    id: 'm11',
    name: 'Samir Gupta',
    age: 30,
    city: 'Kolkata',
    rank: 'Percentile 7.8%',
    status: 'Sealed',
    bio: 'Neurosurgeon. Life is a delicate balance of surgery and poetry.',
    img: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?q=80&w=1287&auto=format&fit=crop',
    height: 1500,
    height_str: "5'11\"",
    vocation: 'Neurosurgeon',
    tier: 'Clinical Asset Tier'
  },
  {
    id: 'm12',
    name: 'Varun Das',
    age: 27,
    city: 'Chennai',
    rank: 'Percentile 12.1%',
    status: 'Rising',
    bio: 'Marine biologist. The ocean holds the secrets I strive to learn.',
    img: 'https://images.unsplash.com/photo-1539571696357-5a69c17a67c6?q=80&w=1287&auto=format&fit=crop',
    height: 1400,
    height_str: "5'10\"",
    vocation: 'Marine Biologist',
    tier: 'Oceanic Signal Tier'
  },
  {
    id: 'm13',
    name: 'Aditya Rao',
    age: 31,
    city: 'London',
    rank: 'Percentile 0.5%',
    status: 'Imperial',
    bio: 'Hedge fund manager. Strategy is the core of every success.',
    img: 'https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?q=80&w=1287&auto=format&fit=crop',
    height: 1650,
    height_str: "6'2\"",
    vocation: 'Hedge Fund Manager',
    tier: 'Strategic Asset Tier'
  },
  {
    id: 'm14',
    name: 'Abhishek Roy',
    age: 29,
    city: 'Delhi',
    rank: 'Percentile 2.9%',
    status: 'Vanguard',
    bio: 'Journalist. Uncovering truths is my life mission.',
    img: 'https://images.unsplash.com/photo-1534030347209-467a5b0ad3e6?q=80&w=1287&auto=format&fit=crop',
    height: 1580,
    height_str: "6'0\"",
    vocation: 'Journalist',
    tier: 'Truth Equity Tier'
  },
  {
    id: 'm15',
    name: 'Pranav Joshi',
    age: 34,
    city: 'Pune',
    rank: 'Percentile 5.4%',
    status: 'Sealed',
    bio: 'Philosopher and author. Every soul is a complex architecture.',
    img: 'https://images.unsplash.com/photo-1542909168-82c3e7fdca5c?q=80&w=1287&auto=format&fit=crop',
    height: 1720,
    height_str: "6'3\"",
    vocation: 'Philosopher/Author',
    tier: 'Intellectual Asset Tier'
  },
  {
    id: 'm16',
    name: 'Karan Mehra',
    age: 25,
    city: 'Surat',
    rank: 'Percentile 15.0%',
    status: 'Rising',
    bio: 'Diamond merchant. Refined taste, unmatched ambition.',
    img: 'https://images.unsplash.com/photo-1504257432389-52343af06ae3?q=80&w=1287&auto=format&fit=crop',
    height: 1480,
    height_str: "5'9\"",
    vocation: 'Diamond Merchant',
    tier: 'Refined Signal Tier'
  },
  {
    id: 'm17',
    name: 'Yash Chopra',
    age: 30,
    city: 'Singapore',
    rank: 'Percentile 1.1%',
    status: 'Imperial',
    bio: 'Digital nomad. Founding the future of work.',
    img: 'https://images.unsplash.com/photo-1513956589380-bad6acb9b9d4?q=80&w=1287&auto=format&fit=crop',
    height: 1680,
    height_str: "6'1\"",
    vocation: 'Digital Nomad Founder',
    tier: 'Future Asset Tier'
  },
  {
    id: 'm18',
    name: 'Zayaan Khan',
    age: 28,
    city: 'Lucknow',
    rank: 'Percentile 6.3%',
    status: 'Vanguard',
    bio: 'Classical poet. Modern heart. Seeking resonance.',
    img: 'https://images.unsplash.com/photo-1522556189639-b150ed9c4330?q=80&w=1287&auto=format&fit=crop',
    height: 1540,
    height_str: "5'11\"",
    vocation: 'Classical Poet',
    tier: 'Lyric Equity Tier'
  },
  {
    id: 'm19',
    name: 'Rishi Khanna',
    age: 32,
    city: 'New York',
    rank: 'Percentile 0.8%',
    status: 'Imperial',
    bio: 'Avenue of the stars. Guided by excellence.',
    img: 'https://images.unsplash.com/photo-1605664041952-4a285090173e?q=80&w=1287&auto=format&fit=crop',
    height: 1780,
    height_str: "6'5\"",
    vocation: 'Private Equity Principal',
    tier: 'Supreme Asset Tier'
  },
  {
    id: 'm20',
    name: 'Ojas Vats',
    age: 29,
    city: 'Ahmedabad',
    rank: 'Percentile 9.2%',
    status: 'Rising',
    bio: 'Textile industrialist. Heritage is the loom of life.',
    img: 'https://images.unsplash.com/photo-1583195764036-6dc248ac07d9?q=80&w=1287&auto=format&fit=crop',
    height: 1510,
    height_str: "6'0\"",
    vocation: 'Textile Industrialist',
    tier: 'Industrial Signal Tier'
  },
  {
    id: 'w1',
    name: 'Ananya Iyer',
    age: 27,
    city: 'New Delhi',
    rank: 'Sovereign Curator',
    status: 'Imperial',
    bio: 'Behavioral psychologist and strategist. Architect of the sovereign mindset.',
    img: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?q=80&w=1287&auto=format&fit=crop',
    height: 168,
    height_str: "5'6\"",
    vocation: 'Behavioral Strategist',
    tier: 'Imperial Curator Tier'
  },
  {
    id: 'w2',
    name: 'Meera Kapoor',
    age: 30,
    city: 'Mumbai',
    rank: 'Lead Resonance Analyst',
    status: 'Imperial',
    bio: 'Art historian and luxury consultant. Guiding the aesthetic evolution of the sanctuary.',
    img: 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?q=80&w=1287&auto=format&fit=crop',
    height: 172,
    height_str: "5'8\"",
    vocation: 'Luxury Consultant',
    tier: 'Sovereign Aesthetic Tier'
  },
  {
    id: 'w3',
    name: 'Sanya Malhotra',
    age: 29,
    city: 'Bangalore',
    rank: 'Protocol Architect',
    status: 'Imperial',
    bio: 'Systems designer and philosopher. Engineering the boundaries of digital intimacy.',
    img: 'https://images.unsplash.com/photo-1531123897727-8f129e16fd3c?q=80&w=1287&auto=format&fit=crop',
    height: 165,
    height_str: "5'5\"",
    vocation: 'Systems Philosopher',
    tier: 'Structural Asset Tier'
  },
  {
    id: 'w4',
    name: 'Isha Verma',
    age: 26,
    city: 'New York',
    rank: 'Global Signal Analyst',
    status: 'Imperial',
    bio: 'Data ethicist and researcher. Investigating the intersection of technology and human dignity.',
    img: 'https://images.unsplash.com/photo-1544005313-94ddf0286df2?q=80&w=1287&auto=format&fit=crop',
    height: 170,
    height_str: "5'7\"",
    vocation: 'Data Ethicist',
    tier: 'Intellectual Prime Tier'
  },
  {
    id: 'w5',
    name: 'Riya Sen',
    age: 31,
    city: 'Dubai',
    rank: 'Selective Intake Lead',
    status: 'Imperial',
    bio: 'Privacy advocate and strategist. Ensuring the absolute discretion of the sanctuary.',
    img: 'https://images.unsplash.com/photo-1524504388940-b1c1722653e1?q=80&w=1287&auto=format&fit=crop',
    height: 167,
    height_str: "5'6\"",
    vocation: 'Privacy Strategist',
    tier: 'Discretion Asset Tier'
  },
  {
    id: 'w6',
    name: 'Tara Khanna',
    age: 28,
    city: 'Singapore',
    rank: 'Aura Calibration Lead',
    status: 'Imperial',
    bio: 'Anthropologist and consultant. Analyzing the microscopic rituals of high-value connection.',
    img: 'https://images.unsplash.com/photo-1506863530036-1efeddceb993?q=80&w=1287&auto=format&fit=crop',
    height: 171,
    height_str: "5'7\"",
    vocation: 'Cultural Anthropologist',
    tier: 'Ritual Asset Tier'
  }
];
