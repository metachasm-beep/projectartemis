...
const identityBlacklist = new Set([
  'Ananya Joshi', 'Sanjana Kapoor', 'Navya Chatterjee', 'Aditi Shetty', 'Navya Rao', 
  'Roshni Reddy', 'Vanya Hegde', 'Kyra Gupta', 'Zoya Hegde', 'Roshni Deshmukh', 
  'Zoya Bajaj', 'Shraddha Sharma', 'Shraddha Patel', 'Ira Kapoor', 'Avani Thakur', 
  'Kyra GUpta', 'Aaira Mehta'
]);
...
async function seedFemales() {
  ...
  console.log("Verifying Registry Purity...");
  const registrySet = new Set(unifiedRegistry);
  if (registrySet.size !== unifiedRegistry.length) {
    console.warn(`WARNING: Registry contains ${unifiedRegistry.length - registrySet.size} duplicates. Filtering...`);
  }
  const uniqueRegistry = Array.from(registrySet);
  ...
  while (i < 200) {
    ...
    // Identity Guard: Ensure no name collisions and exclude blacklisted duplicates
    if (usedNames.has(fullName) || identityBlacklist.has(fullName)) continue;
    ...
    // Absolute Visual Uniqueness
    const photoUrl = uniqueRegistry[i % uniqueRegistry.length];
  ...
  console.log("Sanctuary 2.5.0 Absolute Purity Rectification complete.");
  process.exit(0);
}
