...
import { RefreshCw, Tag, Clock, ChevronDown, IndianRupee, Users, TrendingUp, Loader2, Copy, CheckCircle2, Sparkles, User, Star, Award } from 'lucide-react';
...
/**
 * 🏛️ Influencer Dashboard 2.0: Sovereign Influence Hub (Fold Architecture)
...
export const InfluencerDashboard: React.FC<{ onSwitchToProfile?: () => void }> = ({ onSwitchToProfile }) => {
  ...
  const [activeFold, setActiveFold] = useState('hub');

  // Synchronize with ?fold= URL param
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const fold = params.get('fold');
    if (fold && ['hub', 'transmission', 'chronicle'].includes(fold)) {
      setActiveFold(fold);
    }
  }, []);
  ...
  const foldsConfig = [
    { id: 'hub', label: 'Influence.Hub', icon: Star },
    { id: 'transmission', label: 'Sacred.Code', icon: Tag },
    { id: 'chronicle', label: 'Referral.Ledger', icon: Clock },
  ];
  ...
  return (
    <motion.div
      ...
      className="relative isolate w-full h-[100dvh] overflow-hidden bg-mat-obsidian text-mat-bone selection:bg-mat-rose selection:text-white flex flex-col justify-between pb-36 md:pb-28"
    >
      ...
      <div className="flex-1 w-full flex items-center justify-center overflow-y-auto my-4 px-2 md:px-4">
        <AnimatePresence mode="wait">
          {activeFold === 'hub' && (
            <motion.div key="hub" variants={foldVariants} initial="initial" animate="animate" exit="exit" className="w-full max-w-6xl mx-auto space-y-12 py-4">
              {/* Cinematic Hero */}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 sm:gap-8 z-10 relative px-4">
          ...
          {activeFold === 'transmission' && (
            <motion.div key="transmission" variants={foldVariants} initial="initial" animate="animate" exit="exit" className="w-full max-w-5xl mx-auto py-4 flex flex-col justify-center items-center">
              {/* Massive Promo Banner (Sacred Transmission Code) */}
              <div className="w-full mat-glass-deep p-8 sm:p-12 md:p-16 rounded-[3rem] sm:rounded-[4rem] border border-mat-gold/20 bg-gradient-to-b from-white/5 to-transparent relative overflow-hidden shadow-[0_0_100px_rgba(0,0,0,0.8)] text-center space-y-6 sm:space-y-8 z-10">
          ...
          {activeFold === 'chronicle' && (
            <motion.div key="chronicle" variants={foldVariants} initial="initial" animate="animate" exit="exit" className="w-full max-w-6xl mx-auto py-4 flex flex-col justify-center items-center">
              {/* Cinematic Ledger */}
              <div className="w-full p-6 sm:p-10 md:p-12 rounded-[2.5rem] sm:rounded-[4rem] bg-white/5 border border-white/10 backdrop-blur-2xl space-y-6 sm:space-y-8 shadow-2xl z-10 relative">
