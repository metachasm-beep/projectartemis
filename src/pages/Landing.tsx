...
const LandingPage: React.FC = () => {
  ...
  React.useEffect(() => {
    const checkSession = async () => {
      ...
      if (session?.user) {
        ...
        // Check if profile is complete (has DOB)
  ...
  const handleOnboardingComplete = async (data: { dob: string; analytics: boolean; ads: boolean }) => {
    ...
    try {
      // 1. Log Statutory Consent (Forensic)
      ...
      // 2. Persist DOB and Granular Consent
      ...
      if (!response.ok) {
        ...
        throw new Error(errData.detail || "Statutory validation failed.");
      }

      setShowOnboarding(false);
      // Redirect to dashboard based on role or simple reload
              ...
              <div className="p-12 space-y-8 relative">
                ...
                <div className="space-y-4">
                  <button
                    ...
                    className="w-full h-14 flex items-center justify-center gap-4 rounded-2xl font-bold text-sm transition-all shadow-lg active:scale-[0.98]"
                    style={{
                      background: (isLoading || !agreedToTerms) ? 'rgba(201,160,154,0.3)' : 'linear-gradient(135deg, #7B2D42, #96404F)',
                      color: 'white',
                      fontFamily: 'Helvetica,sans-serif',
                      boxShadow: (!isLoading && agreedToTerms) ? '0 8px 32px rgba(123,45,66,0.25)' : 'none',
                  ...
                  <div className="flex items-start gap-3 px-1 group cursor-pointer" onClick={() => setAgreedToTerms(!agreedToTerms)}>
                    <div className={`w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all shrink-0 mt-0.5 ${agreedToTerms ? 'bg-mat-wine border-mat-wine' : 'border-mat-wine/20 bg-white'}`}>
